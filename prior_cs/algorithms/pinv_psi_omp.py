...
def design_pinv_psi(Phi, X):
    """
    针对给定的信号 X 计算最优传感矩阵 Psi (Oracle Solution)。
    公式: Psi = (Phi * X * X^T * Phi^T)^-1 * (Phi * X * X^T)
    """
    # ====================================================
    # [核心修复] 强制将 X 转为二维列向量 (N, 1)
    # 这一步必须在计算 A 之前做！
    # ====================================================
    if X.ndim == 1:
        X = X.reshape(-1, 1)
    ...
    # 4. 计算 Psi
    # 使用 pinv (伪逆) 处理奇异矩阵。
    # 当针对单个样本计算时，AA_T 的秩仅为 1，直接求逆会报错，必须用 pinv。
    try:
        AA_T_inv = np.linalg.pinv(AA_T)
    except np.linalg.LinAlgError:
        # 极端的数值保护
        AA_T_inv = np.linalg.pinv(AA_T + 1e-10 * np.eye(AA_T.shape[0]))
        
    Psi = np.dot(AA_T_inv, AX_T)
...
def pinv_omp(Phi, y, X, k):
    # 计算 Psi 矩阵
    Psi = design_pinv_psi_fast(Phi, X)
