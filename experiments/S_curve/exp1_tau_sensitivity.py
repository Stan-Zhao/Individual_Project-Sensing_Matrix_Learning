...
DB_FILE = "exp1_tau_structured_results.npy"

# 模型维度配置
N, M = 256, 64
# [核心自变量]: 稀疏度测试范围 (可方便修改)
X_TEST_POINTS = [45,50,55,60,65,70,75]
...
X_train = generate_train_data(N, 1000, max(X_TEST_POINTS) + 5, alpha=1.0)
        ...
        for i, k in enumerate(X_TEST_POINTS):
            ...
            for _ in range(PHI_REPEATS):
                ...
                for tau in TAU_LIST:
                    key = format_tau(tau)
                    # 运行你的算法
                    x_est = pinv_omp(Phi, y, X_train, k, tau)
