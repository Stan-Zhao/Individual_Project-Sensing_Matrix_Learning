...
DB_FILE = "exp4_noise_structured_results.npy"

# 模型维度配置 (固定 N, M, K，只改变噪声)
N, M = 256, 64
K_FIXED = 30
# [核心自变量]: 信噪比 SNR (dB) 测试范围
X_TEST_POINTS = [5,10, 15, 20, 25, 30, 35, 40]
# [对比方法]: 
...
def calc_metrics(x_true, x_est):
    """计算 SRR 和 NRE"""
    # OMP 算法未被选中的原子系数严格为 0，阈值设为 1e-5 足够安全
...
# ==================== 3. 主逻辑 ====================
# 提前学习的训练集 (保证涵盖 K_FIXED)
X_train = generate_train_data(N, 1000, K_FIXED + 20, alpha=1.0)
...
if NEW_TRIALS > 0:
    for trial in range(NEW_TRIALS):
        ...
        # 严格使用结构化数据作为本次测试真值 (K 固定为 K_FIXED)
        x_true = generate_structured_power_law(N, K_FIXED, alpha=1.0)
        
        for i, snr in enumerate(X_TEST_POINTS):
            for _ in range(PHI_REPEATS):
                ...
                # 根据 SNR(dB) 计算噪声功率: SNR = 10 * log10(P_signal / P_noise)
                P_noise = P_signal / (10 ** (snr / 10.0))
                # 生成高斯白噪声
                noise = np.random.randn(M) * np.sqrt(P_noise)
                y_noisy = y_clean + noise
                
                # 1. Gaussian (Standard OMP) 在含噪数据上恢复
                x_g = pro_omp_solve(Phi, Phi, y_noisy, K_FIXED)
                
                # 2. Schnass (POCS Sensing Dictionary)
                Psi_s = get_schnass_sensing_dictionary_pocs(Phi, max_iter=20)
                x_s = pro_omp_solve(Phi, Psi_s, y_noisy, K_FIXED)
                
                # 3. Proposed (Pinv OMP with Prior)
                x_p = pinv_omp(Phi, y_noisy, X_train, K_FIXED, TAU_OPT)
